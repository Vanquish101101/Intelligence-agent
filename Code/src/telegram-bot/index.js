...
// handlerTimeout: Telegraf по умолчанию считает обработчик "зависшим" через
// 90 секунд (найдено живым тестом 2026-07-13 — /trends по теме "инвестиции
// в РФ" реально шёл ~200с; Telegraf в это время уже сообщил пользователю
// "Внутренняя ошибка агента", хотя orchestrate() продолжал работать в фоне
// и в итоге прислал настоящий отчёт — таймаут НЕ останавливает промис,
// только считает его ошибочным раньше времени). deep-режим по документации
// и так заявлен на 2–5 минут — с дефолтным лимитом попадал бы в эту же
// ловушку систематически, не только в исключительных случаях. Поднято с
// большим запасом: 10 минут.
const bot = new Telegraf(process.env.TELEGRAM_BOT_TOKEN, { handlerTimeout: 10 * 60 * 1000 });

// ──────────────────────────────────────────────────────
// Global error handler
// ──────────────────────────────────────────────────────
// Разделяет два принципиально разных случая (найдено живым тестом
// 2026-07-13, см. "06. Задачник.md"):
//   1. TimeoutError от handlerTimeout — обработчик НЕ упал, просто ещё не
//      уложился в 10 минут; сам промис продолжает работать в фоне и почти
//      всегда пришлёт настоящий результат следом. Пугающее "внутренняя
//      ошибка" здесь неправда — честнее сказать "жди дальше".
//   2. Реальное исключение (баг, сеть недоступна и т.п.) — тут "внутренняя
//      ошибка" уместна как есть.
// Настоящего определителя "завис навсегда" не существует — JS не может
// заглянуть в промис и понять, вернётся ли он вообще; единственный честный
// способ отличить "завис навсегда" от "просто долго" — принудительная
// отмена по жёсткому дедлайну (AbortController через весь orchestrate()),
// отдельная, более крупная доработка, если понадобится.
function isHandlerTimeout(err) {
...
bot.catch((err, ctx) => {
  ...
  if (isHandlerTimeout(err)) {
    ctx?.reply(
      '⏳ Запрос обрабатывается дольше обычного — поиск/анализ может занимать пару минут в ' +
      'зависимости от задачи. Это не ошибка, просто дольше, чем всегда: результат обычно всё ' +
      'равно приходит следом. Если через несколько минут так ничего и не пришло — попробуй ещё раз.'
    ).catch(() => {});
    return;
  }
